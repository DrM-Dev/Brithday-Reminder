...
def display_nearest_birthday():
    ...
    # ------------------------------
    if date_data != ("",0):
        if date_data[1] == "0" or date_data[1] == 0:
            nearest_date_data = f">> TODAY IS {date_data[0]}'s BIRTHDAY! <<"
            nearest_bday_display.configure(text=nearest_date_data)
            nearest_bday_display.place(x=widgets_x_place+100, y=widgets_y_place + 25)
        else:
